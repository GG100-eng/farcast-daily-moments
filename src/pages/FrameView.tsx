...
const FrameView = () => {
  const [promptStatus, setPromptStatus] = useState<{ isActive: boolean; remainingTime: number } | null>(null);
  const [loading, setLoading] = useState(true);
  
  // The base URL for production - change this to your actual deployment URL when deploying
  ...
  // Currently the frame image is dynamic based on the prompt status 
  // In a real implementation, this would be a server-side API endpoint generating an image
  const frameImage = promptStatus?.isActive 
    ? `${baseUrl}/prompt-active.png` 
    : `${baseUrl}/prompt-inactive.png`;
  ...
  return (
    <>
      <Helmet>
        ...
        <meta property="fc:frame:button:1:target" content={`${baseUrl}/redirect/camera`} />
        <meta property="fc:frame:button:2" content="View Timeline" />
        <meta property="fc:frame:button:2:action" content="post_redirect" />
        <meta property="fc:frame:button:2:target" content={`${baseUrl}/redirect/home`} />
      </Helmet>
      ...
      <div className="flex flex-col items-center justify-center p-4 max-w-md mx-auto">
          ...
          <div className="text-center w-full p-4 bg-gray-100 rounded-lg mb-6">
            {promptStatus.isActive ? (
              <>
                ...
                <p className="mb-2">You have {Math.floor(promptStatus.remainingTime / 60)}:{String(promptStatus.remainingTime % 60).padStart(2, '0')} to capture your moment</p>
        ...
        <div className="flex gap-4 w-full">
          <Button className="flex-1" variant="outline">View Timeline</Button>
        </div>
